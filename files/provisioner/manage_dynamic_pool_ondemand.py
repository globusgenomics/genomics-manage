...
worker_name_ondemand = "worker@{0}_ondemand".format(instance)
...
print "[{0}] ---Provisioner Start (on demand function added)---".format(datetime.datetime.now())
...
# check num of on demand instances
response = client.describe_instances(
        Filters=[
            {
                "Name": "instance-state-name",
...
if nodes > 0:
    ...
    with open(user_data_file["r4.8xlarge"], "r") as r:
        user_data = r.read()
    ...
    response = ec2.create_instances(
                ...
                SubnetId= az_to_subnet[availability_zone],
